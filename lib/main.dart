...
// ===== HOME SCREEN - ONLY ONE SCAFFOLD IN WHOLE APP =====
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget getBody() {
    switch(currentIndex){
      case 0: return ConnectTab(key: connectKey);
      case 1: return JobsTab(key: jobsKey);
      case 2: return LodgesTab(key: lodgesKey);
      case 3: return const ProfileTab();
      default: return ConnectTab(key: connectKey);
    }
  }
  ...
  Future<void> _initUser() async {
    try {
      ...
      if (myUid!= null &&!myUid!.startsWith('local_')) {
        try {
          await FirebaseFirestore.instance.collection('users').doc(myUid).set({'name': myName, 'stateBatch': sp.getString('stateBatch')?? 'Oyo State', 'ppa': sp.getString('ppa')?? '', 'skills': sp.getString('skills')?? '', 'uid': myUid, 'lastSeen': FieldValue.serverTimestamp()}, SetOptions(merge: true));
        } catch (_) {}
      }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Container(color: Colors.white, padding: const EdgeInsets.all(12), child: StreamBuilder<QuerySnapshot>(stream: FirebaseFirestore.instance.collection('posts').orderBy('createdAt', descending: true).limit(10).snapshots(), builder: (context, snap) {
        if (snap.hasError) return Text("Enable Anonymous in Firebase", style: TextStyle(fontSize:11, color: Colors.red[700]));
      ...
      Expanded(child: TabBarView(controller: _tabController, children: [
        StreamBuilder<QuerySnapshot>(stream: FirebaseFirestore.instance.collection('users').snapshots(), builder: (ctx,snap){ if(snap.hasError) return Center(child: Text("Error: ${snap.error}")); if(!snap.hasData) return const Center(child: CircularProgressIndicator()); var users=snap.data!.docs.where((d)=> d.id!=myUid).toList(); if(search.isNotEmpty){ users=users.where((d){ var m=d.data() as Map<String,dynamic>; return (m['name']??'').toString().toLowerCase().contains(search.toLowerCase()) || (m['ppa']??'').toString().toLowerCase().contains(search.toLowerCase()); }).toList(); } if(users.isEmpty) return const Center(child: Text("No corpers found")); return ListView.builder(itemCount: users.length, itemBuilder: (ctx,i){ var u=users[i].data() as Map<String,dynamic>; return Card(child: ListTile(leading: CircleAvatar(backgroundColor: Colors.green[100], child: Text((u['name']??'C')[0].toUpperCase())), title: Text(u['name']??'Corper'), subtitle: Text("${u['stateBatch']??''} - ${u['ppa']??''}\n${u['skills']??''}"), isThreeLine:true, trailing: ElevatedButton(onPressed: ()=> sendRequest(u['uid'], u['name']??'Corper'), style: ElevatedButton.styleFrom(backgroundColor: Colors.green[700], foregroundColor: Colors.white), child: const Text("Add")), onTap: ()=> Navigator.push(context, MaterialPageRoute(builder: (_)=> UserProfileView(userData: u))))); }); }),
