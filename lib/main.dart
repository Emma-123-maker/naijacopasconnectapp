...
// ===== NEW CONNECT TAB - SEARCH + FRIEND REQUEST + CHAT + PROFILE =====
...
class _ConnectTabState extends State<ConnectTab> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initUser() async {
    ...
    String? img = sp.getString('profile_image');
    if (myUid!= null) {
      await FirebaseFirestore.instance.collection('users').doc(myUid).set({'name': myName, 'stateBatch': stateBatch, 'ppa': ppa, 'skills': skills, 'uid': myUid, 'hasPhoto': img!=null, 'lastSeen': FieldValue.serverTimestamp()}, SetOptions(merge: true));
    }
    setState(() {});
  }
  ...
  String getChatId(String otherUid) => myUid!.compareTo(otherUid) < 0? "${myUid}_$otherUid" : "${otherUid}_${myUid}";
      ...
      body: Column(children: [
        ...
        Expanded(child: TabBarView(controller: _tabController, children: [
          ...
          StreamBuilder<QuerySnapshot>(stream: FirebaseFirestore.instance.collection('users').snapshots(), builder: (ctx, snap) {
            ...
            if (users.isEmpty) return const Center(child: Text("No corpers found. Invite friends to install app!"));
          ...
          StreamBuilder<QuerySnapshot>(stream: FirebaseFirestore.instance.collection('users').doc(myUid).collection('friends').snapshots(), builder: (ctx,snap){
            if(!snap.hasData) return const Center(child: CircularProgressIndicator());
            if(snap.data!.docs.isEmpty) return const Center(child: Text("No friends yet. Go to Discover and send request!"));
            return ListView.builder(itemCount: snap.data!.docs.length, itemBuilder: (ctx,i){ var m = snap.data!.docs[i].data() as Map<String,dynamic>; return Card(child: ListTile(leading: CircleAvatar(backgroundColor: Colors.green[100], child: Text((m['name']??'C')[0].toUpperCase())), title: Text(m['name']??''), subtitle: const Text("Friends - Tap to chat / view profile"), trailing: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(icon: const Icon(Icons.person, color: Colors.blue), onPressed: () async { var userDoc = await FirebaseFirestore.instance.collection('users').doc(m['uid']).get(); if (userDoc.exists) { if(context.mounted) Navigator.push(context, MaterialPageRoute(builder: (_)=> UserProfileView(userData: userDoc.data() as Map<String,dynamic>))); } }), const Icon(Icons.chat, color: Colors.green)]), onTap: ()=> Navigator.push(context, MaterialPageRoute(builder: (_)=> ChatScreen(otherUid: m['uid'], otherName: m['name'], myUid: myUid!))))); });
          }),
          // CHATS
          StreamBuilder<QuerySnapshot>(stream: FirebaseFirestore.instance.collection('chats').where('participants', arrayContains: myUid).orderBy('updatedAt', descending: true).snapshots(), builder: (ctx,snap){
            if(!snap.hasData) return const Center(child: CircularProgressIndicator());
            if(snap.data!.docs.isEmpty) return const Center(child: Text("No chats yet. Accept friend request to start chat"));
  ...
  @override Widget build(BuildContext context){
    return Scaffold(appBar: AppBar(title: Text(widget.otherName), backgroundColor: Colors.green[700], foregroundColor: Colors.white, actions: [IconButton(icon: const Icon(Icons.person), onPressed: () async { var userDoc = await FirebaseFirestore.instance.collection('users').doc(widget.otherUid).get(); if (userDoc.exists && context.mounted) Navigator.push(context, MaterialPageRoute(builder: (_)=> UserProfileView(userData: userDoc.data() as Map<String,dynamic>))); })]), body: Column(children: [
      ...
      Container(padding: const EdgeInsets.all(8), color: Colors.white, child: Row(children: [
        ...
        IconButton(icon: const Icon(Icons.mic, color: Colors.orange), onPressed: ()=> ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Voice note coming in +48 - use text & photo for now 🎤")))),
...
class _JobsTabState extends State<JobsTab> {
  ...
  void addJobDialog() {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setD) => AlertDialog(title: const Text("Post Job Vacancy"), content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: titleCtrl, decoration: const InputDecoration(labelText: "Job Title *", hintText: "e.g Maths Teacher, Sales Rep")), const SizedBox(height: 8), TextField(controller: companyCtrl, decoration: const InputDecoration(labelText: "Company / Organization", hintText: "e.g UI Sec School")), const SizedBox(height: 8), DropdownButtonFormField<String>(value: jobType, decoration: const InputDecoration(labelText: "Job Type *"), items: const [DropdownMenuItem(value: "Teaching / Lesson", child: Text("Teaching / Lesson")), DropdownMenuItem(value: "PPA Opening", child: Text("PPA Opening")), DropdownMenuItem(value: "Full-time", child: Text("Full-time")), DropdownMenuItem(value: "Part-time / Side Hustle", child: Text("Part-time / Side Hustle")), DropdownMenuItem(value: "Freelance / Remote", child: Text("Freelance / Remote")), DropdownMenuItem(value: "Sales / Marketing", child: Text("Sales / Marketing"))], onChanged: (v)=> setD(()=> jobType = v!)), const SizedBox(height: 8), TextField(controller: locationCtrl, maxLines: 3, decoration: const InputDecoration(labelText: "Job Location Details *", hintText: "Where is the job located? Full details", border: OutlineInputBorder())), const SizedBox(height: 8), TextField(controller: payCtrl, decoration: const InputDecoration(labelText: "Pay / Salary", hintText: "e.g ₦30k/month")), const SizedBox(height: 8), TextField(controller: reqCtrl, maxLines: 3, decoration: const InputDecoration(labelText: "Requirements", hintText: "What is needed for this job?", border: OutlineInputBorder())), const SizedBox(height: 8), TextField(controller: descCtrl, maxLines: 4, decoration: const InputDecoration(labelText: "Full Job Description *", hintText: "Write EVERY information about this vacancy here...", border: OutlineInputBorder())), const SizedBox(height: 8), TextField(controller: contactCtrl, decoration: const InputDecoration(labelText: "WhatsApp / Contact *"), keyboardType: TextInputType.phone)])), actions: [TextButton(onPressed: ()=> Navigator.pop(ctx), child: const Text("Cancel")), ElevatedButton(onPressed: (){ if(titleCtrl.text.isEmpty || locationCtrl.text.isEmpty || descCtrl.text.isEmpty || contactCtrl.text.isEmpty){ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Title, Location, Description & WhatsApp required"))); return; } setState(()=> jobs.insert(0, {"title": titleCtrl.text, "company": companyCtrl.text, "location": locationCtrl.text, "type": jobType, "pay": payCtrl.text, "requirement": reqCtrl.text, "desc": descCtrl.text, "contact": contactCtrl.text, "postedBy": "You", "date": "Now"})); saveJobs(); Navigator.pop(ctx); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.green[700], foregroundColor: Colors.white), child: const Text("Post Job"))])));
  }
...
class _LodgesTabState extends State<LodgesTab> {
  ...
  void addLodgeDialog() {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setD) => AlertDialog(title: const Text("Post Lodge"), content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [DropdownButtonFormField<String>(value: category, decoration: const InputDecoration(labelText: "Post Type *"), items: const [DropdownMenuItem(value: "I NEED Lodge - Looking for apartment", child: Text("I NEED - Apartment")), DropdownMenuItem(value: "I NEED Roommate to join me", child: Text("I NEED - Roommate")), DropdownMenuItem(value: "I HAVE Lodge Info", child: Text("I HAVE - Lodge Info"))], onChanged: (v)=> setD(()=> category = v!)), const SizedBox(height: 10), TextField(controller: areaCtrl, decoration: const InputDecoration(labelText: "Area *", hintText: "Bodija, Akobo, Sango")), const SizedBox(height: 8), TextField(controller: locationCtrl, maxLines: 3, decoration: const InputDecoration(labelText: "Location Details *", hintText: "Type full location: e.g Behind FoodCo Bodija, close to UI", border: OutlineInputBorder())), const SizedBox(height: 8), TextField(controller: priceCtrl, decoration: const InputDecoration(labelText: "Price / Budget *")), TextField(controller: typeCtrl, decoration: const InputDecoration(labelText: "House Type", hintText: "Self-con, Single room")), TextField(controller: descCtrl, maxLines: 2, decoration: const InputDecoration(labelText: "More Info")), TextField(controller: contactCtrl, decoration: const InputDecoration(labelText: "WhatsApp Number *"), keyboardType: TextInputType.phone)])), actions: [TextButton(onPressed: ()=> Navigator.pop(ctx), child: const Text("Cancel")), ElevatedButton(onPressed: (){ if(areaCtrl.text.isEmpty || locationCtrl.text.isEmpty || contactCtrl.text.isEmpty){ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Area, Location Details & WhatsApp required"))); return; } setState(()=> lodges.insert(0, {"category": category, "area": areaCtrl.text, "location": locationCtrl.text, "price": priceCtrl.text, "type": typeCtrl.text, "desc": descCtrl.text, "contact": contactCtrl.text, "postedBy": "You", "date": "Now"})); saveLodges(); Navigator.pop(ctx); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.green[700], foregroundColor: Colors.white), child: const Text("Post"))]));
  }
